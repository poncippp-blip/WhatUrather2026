...
class VideoGenerator {
    ...
    setupEnhancedControls() {
        ...
        document.querySelectorAll('input[name="promptMode"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                this.simpleFastFoodMode = (value === 'simpleFoods');
                this.triggerAutoSave();
            });
        });
    ...
    saveConfig() {
        const config = {
            ...
            simpleFastFoodMode: this.simpleFastFoodMode,
    ...
    async loadConfig(event) {
        ...
        try {
            ...
            // Load prompt mode (food-only mode or simple fast food mode)
            if (config.foodOnlyMode !== undefined || config.simpleFastFoodMode !== undefined) {
                this.foodOnlyMode = config.foodOnlyMode || false;
                this.simpleFastFoodMode = config.simpleFastFoodMode || false;

                // Set the appropriate radio button
                let promptModeValue = 'all';
                if (this.simpleFastFoodMode) {
                    promptModeValue = 'simpleFoods';
                } else if (this.foodOnlyMode) {
                    promptModeValue = 'allFoods';
                }

                const radioButton = document.querySelector(`input[name="promptMode"][value="${promptModeValue}"]`);
                if (radioButton) {
    ...
    async downloadVideo() {
        ...
        try {
            ...
            // Use local FFmpeg core files to avoid CORS/Worker issues
            const baseURL = `${window.location.origin}/src/libs/ffmpeg/core`;
            await ffmpeg.load({
